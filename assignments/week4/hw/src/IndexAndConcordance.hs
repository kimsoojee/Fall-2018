...
cleanup str = map toLower (checkPunctuation punctuation str)
checkPunctuation :: String -> String -> String
checkPunctuation punc [] = []
checkPunctuation punc (x:xs) = if elem x punc
                               then " " ++ checkPunctuation punc xs
                               else [x] ++ checkPunctuation punc xs
...
createWordLists strList = map (\a -> words a) (map (\a -> cleanup a) strList)
...
makePairs strList = zip ([1..(countList strList)]) strList
countList :: [[String]] -> Integer
countList strList2 = sum(map (\a -> 1) strList2)
...
addPageNumber (int,str)  = map (\a -> (int,a)) str
...
createWordList strList =  concat (map (\a -> addPageNumber a) (makePairs (createWordLists strList)))


-- Now write a function to insert a list of pairs Integero an index tree

insertAll :: [(Integer, String)] ->  IndexTree Integer String
insertAll list = foldr insertHelper Null list
insertHelper :: (Integer, String) ->  IndexTree Integer String ->  IndexTree Integer String
insertHelper (i,s) tree = IndexTree.insert i s tree


-- Now create the tree and then invert it

makeConcordance b = invert (insertAll (createWordList b))
...
-- tree = Node (Node (Node Null 1 ["page","first","the","is","start","to","this"] Null) 2 ["two","of","page","second","the","is","this"] Null) 3 ["three","of","third","the","is","this"] Null
