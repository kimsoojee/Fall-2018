...
instance Functor Parser where
  ...
  fmap f (Parser pa) =  Parser $ \string -> case pa string of
                                                Nothing -> Nothing
...
instance Monad Parser where
  --return :: a -> Parser a
  return a =  Parser $ \string -> Just (a, string)

  --(>>=) :: Parser a -> (a -> Parser b) -> Parser b
  (Parser pa) >>= f  = Parser $ \string -> case pa string of
                                              Nothing -> Nothing
