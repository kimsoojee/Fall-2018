...
stdLib = fromList [("tail", Fun $ \ v -> case v of Ls (_:ls)-> Ok $ Ls ls
                   ...
                   ("head", Fun $ \ v -> case v of Ls (s:_)-> Ok $ s
                                                   _ -> Error ""),
                   ("len", Fun $ \ v -> case v of Ls x -> Ok $ I (toInteger(length x))
                                                  _ -> Error "")  ]
...
eval (And l r) = do lres <- evalBool l
                    rres <- evalBool r
                    return $ B $ lres && rres
eval (Or l r) = do lres <- evalBool l
                   rres <- evalBool r
                   return $ B $ lres || rres
eval (Not ast) = do a <- evalBool ast
                    return $ B $ not a
eval (ValInt int) = return $ I int
eval (Plus l r) = do lres <- evalInt l
                     rres <- evalInt r
                     return $ I $ lres + rres
eval (Minus l r) = do lres <- evalInt l
                      rres <- evalInt r
                      return $ I $ lres - rres
eval (Mult l r) = do lres <- evalInt l
                     rres <- evalInt r
                     return $ I $ lres * rres
eval (Div l r) = do lres <- evalInt l
                    rres <- evalInt r
                    case rres of 0 -> err "cannot divide by 0"
                                 _ -> return $ I $ lres `div` rres
eval (Nil) = return $ Ls []
eval (Cons a b) = do ares <- eval a
                     bres <- evalLs b
                     return $ Ls $ ares : bres

eval (If a b c) = do ares <- evalBool a
                     case ares of True -> eval b
                                  False -> eval c
eval (Let string a b) = do aeval <- eval a
                           withVal string aeval (eval b)
eval (Var string) = valOf string
eval (Lam s bod) = do env <- getEnv
                      return $ Fun $ \v -> r (eval bod) (insert s v env)
eval (App f a) = do f' <- evalFun f
                    a' <- eval a
                    case f' a' of Error s -> err s
                                  Ok b -> return b
...
evalBool a = do res <- eval a
                case res of B b -> return b
                            _   -> err "not the bool we expected"

evalLs :: Ast -> EnvUnsafe Env [Val]
evalLs a = do res <- eval a
              case res of Ls l -> return l
                          val  -> return [val]
                          -- err "not the list we expected"

evalFun :: Ast -> EnvUnsafe Env (Val -> Unsafe Val)
evalFun a = do res <- eval a
               case res of Fun f  -> return f
                           val -> return (\v -> Ok val)
...
e7 = prettyShow ((Var "fun") `App` (ValInt 2)) $ 0
e8 = prettyShow (((Lam "x" (Lam "y" ( (Var "x") `Plus` (Var "y")))) `App` (ValInt 7)) `App` (ValInt 4)) $ 0
e9 = prettyShow (let x = Var "x" in  (Lam "x" ( x `Plus` x)) `App` (ValInt 7) ) $ 0
