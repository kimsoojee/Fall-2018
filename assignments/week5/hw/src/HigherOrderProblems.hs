...
sumsqa n = foldr sumHelper 0 n
sumHelper :: Integer -> Integer -> Integer
sumHelper int sumInt = (int ^ 2) + sumInt
...
remdups :: Eq a => [a] -> [a]
remdups lst = foldr remHelper [] lst
remHelper :: Eq a => a -> [a] -> [a]
remHelper x list = if list == []
                     then [x]
                     else if x == (head list)
                          then list
                          else [x] ++ list
...
inits :: [a] -> [[a]]
inits list = foldr initHelper [[]] (reverse list)
initHelper :: a -> [[a]] -> [[a]]
initHelper x list2 = list2 ++ [(last list2) ++ [x]]
